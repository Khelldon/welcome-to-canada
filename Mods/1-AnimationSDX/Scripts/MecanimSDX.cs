...
class MecanimSDX : AvatarController
{
    ...
    private bool useModderUpdate = true;
    ...
    private MecanimSDX()
    {
        ...
        // Use this attribute to enable the legacy movement
        // <!-- <property name="UseModderUpdate" value="true" />
        bool.TryParse(entityClass.Properties.Values["UseModderUpdate"], out useModderUpdate);
    ...
    public void ModderUpdate()
    {

        // If the modder update is disabled, don't process
        if (!useModderUpdate)
            return;
        ...
        this.SetBool("IsInWater", entity.IsInWater());
        Log("Entity is in Water: " + entity.IsInWater());

        int CurrentMovementState = 0;
        this.TryGetInt("MovementState", out CurrentMovementState);
        Log("Movement State on Animation: " + CurrentMovementState);


        if (entity.Electrocuted)
            StartAnimationElectrocuted();
        else
            this.IsElectrocuting = false;

        if (entity.HarvestingAnimation)
        ...
        if (encroached > 0.150000005960464)
        {
            ...
            if (encroached > 1.0)
                this.SetInt("MovementState", 2);
            else
                this.SetInt("MovementState", 1);
        }
        else
            this.SetInt("MovementState", 0);

        CurrentMovementState = 0;
        this.TryGetInt("MovementState", out CurrentMovementState);
        Log("New Movement State on Animation: " + CurrentMovementState);
    }
